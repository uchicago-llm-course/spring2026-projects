...
Next Steps:
The next step is to stress-test whether this invariance actually survives a broader shift in generators and domains. First, I want to add LLAMA once access is resolved and expand the held-out set so evaluation is less tied to the current five-model pool. Second, I want to run ablations that remove the adversarial heads, the contrastive term, and the cross-model aggregation features one at a time so we can measure which component is really driving the gain over FastDetectGPT. Third, I want to test thresholded metrics like accuracy, precision, recall, and F1 under different operating points, since AUROC alone does not tell us how usable the detector is in practice. If those checks hold up, the final step will be packaging the pipeline into a cleaner end-to-end benchmark so future experiments can add new generators and domains without rebuilding the feature extraction and evaluation stack each time.